...
// bam file
string bamFileName("behavior.boo");

/// specific data/functions declarations/definitions
NodePath sceneNP;
ClockObject* globalClock = NULL;
// camera specifics
WPT(P3Activity)cameraActivity;
// player specifics
...
{
	GameBehaviorManager* behaviorMgr = GameBehaviorManager::get_global_ptr();
	//
	ValueList<string> valueList = behaviorMgr->get_parameter_name_list(
			GameBehaviorManager::SOUND3D);
	cout << endl << "P3Sound3d creation parameters:" << endl;
	for (int i = 0; i < valueList.get_num_values(); ++i)
	{
		cout << "\t" << valueList[i] << " = "
				<< behaviorMgr->get_parameter_value(GameBehaviorManager::SOUND3D,
						valueList[i]) << endl;
	}
	//
	valueList = behaviorMgr->get_parameter_name_list(GameBehaviorManager::LISTENER);
	cout << endl << "P3Activity creation parameters:" << endl;
	for (int i = 0; i < valueList.get_num_values(); ++i)
	{
		cout << "\t" << valueList[i] << " = "
				<< behaviorMgr->get_parameter_value(GameBehaviorManager::LISTENER,
						valueList[i]) << endl;
	}
}
...
void startFramework(int argc, char *argv[], const string& msg)
{
	...
	framework.set_window_title("p3behavior: " + msg);
...
void writeToBamFileAndExit(const Event*, void* data)
{
	...
	WPT(GameBehaviorManager)behaviorMgr = GameBehaviorManager::get_global_ptr();
	// destroy sound3ds
	while (behaviorMgr->get_num_sound3ds() > 0)
	{
		// destroy the first one on every cycle
		behaviorMgr->destroy_sound3d(NodePath::any_path(behaviorMgr->get_sound3d(0)));
///		delete DCAST(P3Sound3d, behaviorMgr->get_sound3d(0).node()); //ERROR
	}
	// destroy listeners
	while (behaviorMgr->get_num_listeners() > 0)
	{
		// destroy the first one on every cycle
		behaviorMgr->destroy_listener(
				NodePath::any_path(behaviorMgr->get_listener(0)));
///		delete DCAST(P3Activity, behaviorMgr->get_listener(0).node()); //ERROR
	}
	//
...
///		delete DCAST(P3Chaser, controlMgr->get_chaser(0).node()); //ERROR
	...
	// delete managers
	delete GameBehaviorManager::get_global_ptr();
...
void listenerCallback(PT(P3Activity)listener)
{
	NodePath refNP =
			GameBehaviorManager::get_global_ptr()->get_reference_node_path();
...
{
	string msg("'P3Sound3d & P3Activity'");
	...
	WPT(GameBehaviorManager)behaviorMgr = new GameBehaviorManager();
	...
	if ((not (argc > 1)) or (not readFromBamFile(argv[1])))
	{
		...
		behaviorMgr->get_reference_node_path().reparent_to(window->get_render());
		...
		NodePath playerSound3dNP = behaviorMgr->create_sound3d("PlayerSound3d");
		NodePath pursuerSound3dNP = behaviorMgr->create_sound3d("PursuerSound3d");
		...
		// set sounds looping
		PT(BehaviorSound)sound = playerSound3d->get_sound_by_name(soundName[4]);
		sound->set_loop(true);
		sound->play();
		//
		...
		NodePath cameraActivityNP = behaviorMgr->create_listener("CameraActivity");
		// get a reference to the camera's listener
		cameraActivity = DCAST(P3Activity, cameraActivityNP.node());
		// reparent listener to the camera
		cameraActivityNP.reparent_to(window->get_camera_group());
	}
	else
	{
		// valid bamFile
		...
		controlMgr->set_reference_node_path(
				behaviorMgr->get_reference_node_path());
		behaviorMgr->get_reference_node_path().reparent_to(window->get_render());
		...
		for (int i = 0; i < behaviorMgr->get_num_sound3ds(); ++i)
		{
			PT(P3Sound3d)sound3d = GameBehaviorManager::get_global_ptr()->get_sound3d(i);

...
from panda3d.core import load_prc_file_data, NodePath, ClockObject, \
                ...
                Filename
from p3ai import GameAIManager, RNCrowdAgent, \
                ValueList_string
import random, sys
...
def loadAllScene():
    ...
    navMesMgr = GameAIManager.get_global_ptr()
    ...
    for i in range(NUMAGENTS):
        ...
        if i % 2 == 0:
            ...
            agentType = "kinematic"
        navMesMgr.set_parameter_value(GameAIManager.CROWDAGENT, "mov_type", agentType)
...
def restoreAllScene():
    ...
    sceneNP = GameAIManager.get_global_ptr().get_reference_node_path().find("**/Owner")
    # reparent the reference node to render
    ...
    for i in range(NUMAGENTS):
        ...
        crowdAgent[i] = GameAIManager.get_global_ptr().get_crowd_agent(i)
        # restore animations
...
def printCreationParameters():
    ...
    navMesMgr = GameAIManager.get_global_ptr()
    #
    valueList = navMesMgr.get_parameter_name_list(GameAIManager.NAVMESH)
    print("\n" + "RNNavMesh creation parameters:")
    for name in valueList:
        print ("\t" + name + " = " + 
               navMesMgr.get_parameter_value(GameAIManager.NAVMESH, name))
    #
    valueList = navMesMgr.get_parameter_name_list(GameAIManager.CROWDAGENT)
    print("\n" + "RNCrowdAgent creation parameters:")
    for name in valueList:
        print ("\t" + name + " = " + 
               navMesMgr.get_parameter_value(GameAIManager.CROWDAGENT, name))
...
def setParametersBeforeCreation():
    ...
    navMesMgr.set_parameter_values(GameAIManager.NAVMESH, "area_flags_cost",
            valueList)
...
def getCollisionEntryFromCamera():
    ...
    navMeshMgr = GameAIManager.get_global_ptr()

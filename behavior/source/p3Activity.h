...
#ifndef P3ACTIVITY_H_
#define P3ACTIVITY_H_
...
/**
 * \brief Component manipulating the activity for 3d sounds.
 *
 * > **XML Param(s)**:
 ...
 */
/**
 * P3Activity is a PandaNode class designed to make an object a activity of another
 * object.
 *
 * P3Activity can be enabled/disabled as a whole (enabled by default).\n
 * P3Activity can handle basic rotations movement (head_left, head_right,
 ...
 * \note parts inside [] are optional.\n
 */
class EXPORT_CLASS P3Activity: public PandaNode
{
PUBLISHED:
	 ...
	 */
	///@{
	typedef void (*UPDATECALLBACKFUNC)(PT(P3Activity));
...
protected:
	friend void unref_delete<P3Activity>(P3Activity*);
	friend class GameBehaviorManager;

	P3Activity(const string& name);
	virtual ~P3Activity();
...
private:
	...
	P3Activity(const P3Activity&);
	P3Activity& operator=(const P3Activity&);
...
public:
	...
	{
		PandaNode::init_type();
		register_type(_type_handle, "P3Activity", PandaNode::get_class_type());
	}

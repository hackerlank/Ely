...
#include <float.h>
#include <new>
#include "AIComponents/RecastNavigationLocal/NavMeshType_Obstacle.h"
#include "AIComponents/RecastNavigationLocal/DebugInterfaces.h"
#include "AIComponents/RecastNavigationLocal/InputGeom.h"
#include "AIComponents/RecastNavigationLocal/ChunkyTriMesh.h"
#include "AIComponents/RecastNavigationLocal/ConvexVolumeTool.h"
#include "AIComponents/RecastNavigationLocal/fastlz.h"
#include <Recast.h>
							   ...
							   const int maxTiles)
{
	...
	ely::FastLZCompressor comp;
	ely::RasterizationContext rc;
	
	const float* verts = geom->getMesh()->getVerts();
	const int nverts = geom->getMesh()->getVertCount();
	const ely::rcChunkyTriMesh* chunkyMesh = geom->getChunkyMesh();
	...
	{
		const ely::rcChunkyTriMeshNode& node = chunkyMesh->nodes[cid[i]];
	...
	// (Optional) Mark areas.
	const ely::ConvexVolume* vols = geom->getConvexVolumes();
	...
	{
		ely::TileCacheData* tile = &rc.tiles[rc.ntiles++];

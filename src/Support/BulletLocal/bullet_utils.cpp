...
#include "Support/BulletLocal/bullet_utils.h"
...
btVector3 LVecBase3_to_btVector3(const LVecBase3 &v)
{
	...
	return btVector3((btScalar) v.get_x(), (btScalar) v.get_y(),
			(btScalar) v.get_z());
}
...
LVecBase3 btVector3_to_LVecBase3(const btVector3 &v)
{
	...
	return LVecBase3((PN_stdfloat) v.getX(), (PN_stdfloat) v.getY(),
			(PN_stdfloat) v.getZ());
}
...
LVector3 btVector3_to_LVector3(const btVector3 &v)
{
	...
	return LVector3((PN_stdfloat) v.getX(), (PN_stdfloat) v.getY(),
			(PN_stdfloat) v.getZ());
}
...
LPoint3 btVector3_to_LPoint3(const btVector3 &p)
{
	...
	return LPoint3((PN_stdfloat) p.getX(), (PN_stdfloat) p.getY(),
			(PN_stdfloat) p.getZ());
}
...
LMatrix3 btMatrix3x3_to_LMatrix3(const btMatrix3x3 &m)
{
	...
	return LMatrix3((PN_stdfloat) cells[0], (PN_stdfloat) cells[1],
			(PN_stdfloat) cells[2], (PN_stdfloat) cells[3],
			(PN_stdfloat) cells[4], (PN_stdfloat) cells[5],
			(PN_stdfloat) cells[6], (PN_stdfloat) cells[7],
			(PN_stdfloat) cells[8]);
}
...
btQuaternion LQuaternion_to_btQuat(const LQuaternion &q)
{
	...
	return btQuaternion((btScalar) q.get_i(), (btScalar) q.get_j(),
			(btScalar) q.get_k(), (btScalar) q.get_r());
}
...
LQuaternion btQuat_to_LQuaternion(const btQuaternion &q)
{
	...
	return LQuaternion((PN_stdfloat) q.getW(), (PN_stdfloat) q.getX(),
			(PN_stdfloat) q.getY(), (PN_stdfloat) q.getZ());
}

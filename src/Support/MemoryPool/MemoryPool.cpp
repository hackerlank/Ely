...
void MemoryPool::Destroy(void)
{
    ...
    std::string str;
    if (m_numAllocs != 0)
        str = "***(" + MP_ToStr(m_numAllocs) + ") ";
    unsigned long totalNumChunks = m_numChunks * m_memArraySize;
    unsigned long wastedMem = (totalNumChunks - m_allocPeak) * m_chunkSize;
    str += "Destroying memory pool: [" + GetDebugName() + ":" + MP_ToStr((unsigned long)m_chunkSize) + "] = " + MP_ToStr(m_allocPeak) + "/" + MP_ToStr((unsigned long)totalNumChunks) + " (" + MP_ToStr(wastedMem) + " bytes wasted)\n";
    ely::OutputDebugStringA(str.c_str());  // the logger is not initialized during many of the initial memory pool growths, so let's just use the OS version
#endif
...
{
#ifdef ELY_DEBUG
    std::string str("Growing memory pool: [" + GetDebugName() + ":" + MP_ToStr((unsigned long)m_chunkSize) + "] = " + MP_ToStr((unsigned long)m_memArraySize + 1) + "\n");
    ely::OutputDebugStringA(str.c_str());  // the logger is not initialized during many of the initial memory pool growths, so let's just use the OS version
#endif

...
void DotSceneLoader::processScene(rapidxml::xml_node<>* XMLRoot)
{
	...
	if (XMLRoot->first_attribute("ID"))
		message += ", id "
				+ Ogre::String(XMLRoot->first_attribute("ID")->value());
	if (XMLRoot->first_attribute("sceneManager"))
		message += ", scene manager "
				+ Ogre::String(
						XMLRoot->first_attribute("sceneManager")->value());
	...
	if (XMLRoot->first_attribute("author"))
		message += ", author "
				+ Ogre::String(XMLRoot->first_attribute("author")->value());
...
void DotSceneLoader::processTerrain(rapidxml::xml_node<>* XMLNode)
{
	...
	int mapSize = Ogre::StringConverter::parseInt(
			XMLNode->first_attribute("mapSize")->value());
	/*int rows =*/
	Ogre::StringConverter::parseInt(XMLNode->first_attribute("rows")->value());
	/*int columns =*/
	Ogre::StringConverter::parseInt(
			XMLNode->first_attribute("columns")->value());
	/*bool colourmapEnabled =*/
	getAttribBool(XMLNode, "colourmapEnabled");
	/*int colourMapTextureSize =*/
	Ogre::StringConverter::parseInt(
...
void DotSceneLoader::processTerrainPage(rapidxml::xml_node<>* XMLNode)
{
	...
	int pageX = Ogre::StringConverter::parseInt(
			XMLNode->first_attribute("pageX")->value());
	int pageY = Ogre::StringConverter::parseInt(
	...
	Ogre::StringConverter::parseInt(
			XMLNode->first_attribute("colourmapTexturesize")->value());
	int layerCount = Ogre::StringConverter::parseInt(
			XMLNode->first_attribute("layerCount")->value());
	...
	else
	{
		...
		while (pElement)
		{
			...
			while (pTerrainLayer)
			{
				int worldSize = Ogre::StringConverter::parseInt(
				...
				{
					imp.layerList[count].textureNames.push_back(
							getAttrib(pTerrainTexture, "name", ""));
...
void DotSceneLoader::processBlendmaps(rapidxml::xml_node<>* XMLNode)
{
			...
			mTerrainGroup->getResourceGroup(), filename))
	{
		...
		for (int j = 1;
				j < mTerrainGroup->getTerrain(pageX, pageY)->getLayerCount();
				j++)
				{
			Ogre::TerrainLayerBlendMap *blendmap = mTerrainGroup->getTerrain(
					pageX, pageY)->getLayerBlendMap(j);
			Ogre::Image img;
			img.load(blendMaps[j - 1], "General");
			int blendmapsize =
					mTerrainGroup->getTerrain(pageX, pageY)->getLayerBlendMapSize();
			if (img.getWidth() != (uint) blendmapsize)
				img.resize(blendmapsize, blendmapsize);

			float *ptr = blendmap->getBlendPointer();
			Ogre::uint8 *data =
					static_cast<Ogre::uint8*>(img.getPixelBox().data);
		...
		Ogre::SceneNode *pParent)
{
	...
	Ogre::String id = getAttrib(XMLNode, "id");
	/*Ogre::Real fov =*/
	getAttribReal(XMLNode, "fov", 45);
	/*Ogre::Real aspectRatio =*/
	getAttribReal(XMLNode, "aspectRatio", 1.3333);
		...
		Ogre::SceneNode *pParent)
{
	...
	try
	{
		Ogre::ParticleSystem *pParticles = mSceneMgr->createParticleSystem(name,
				file);
...
Ogre::Quaternion DotSceneLoader::parseQuaternion(rapidxml::xml_node<>* XMLNode)
{
	...
	{
		Ogre::Vector3 axis;
		axis.x = Ogre::StringConverter::parseReal(
				XMLNode->first_attribute("axisX")->value());
		axis.y = Ogre::StringConverter::parseReal(
				XMLNode->first_attribute("axisY")->value());
		axis.z = Ogre::StringConverter::parseReal(
				XMLNode->first_attribute("axisZ")->value());
	...
	else if (XMLNode->first_attribute("angleX"))
	{
		Ogre::Vector3 axis;
		axis.x = Ogre::StringConverter::parseReal(
				XMLNode->first_attribute("angleX")->value());
		axis.y = Ogre::StringConverter::parseReal(
				XMLNode->first_attribute("angleY")->value());
		axis.z = Ogre::StringConverter::parseReal(
				XMLNode->first_attribute("angleZ")->value());
